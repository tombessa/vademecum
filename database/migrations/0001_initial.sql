...
CREATE TRIGGER trg_legal_unit_search BEFORE INSERT OR UPDATE ON legal_unit
FOR EACH ROW EXECUTE PROCEDURE tsvector_update_trigger(
  search_vector, 'vademecum.portuguese_unaccent', label, heading, body
);
CREATE TRIGGER trg_jurisprudence_search BEFORE INSERT OR UPDATE ON jurisprudence_decision
FOR EACH ROW EXECUTE PROCEDURE tsvector_update_trigger(
  search_vector, 'vademecum.portuguese_unaccent', court, process_number,
  title, holding, full_text
);
CREATE TRIGGER trg_app_user_updated BEFORE UPDATE ON app_user FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_collection_updated BEFORE UPDATE ON legal_collection FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_act_updated BEFORE UPDATE ON normative_act FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_jurisprudence_updated BEFORE UPDATE ON jurisprudence_decision FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_import_request_updated BEFORE UPDATE ON legislation_import_request FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_user_collection_updated BEFORE UPDATE ON user_collection FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_highlight_updated BEFORE UPDATE ON user_highlight FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
CREATE TRIGGER trg_note_updated BEFORE UPDATE ON user_note FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
...
CREATE FUNCTION ensure_chatgpt_user(p_email citext, p_display_name text)
