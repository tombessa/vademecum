...
CREATE TRIGGER legal_unit_search_vector_trigger
    BEFORE INSERT OR UPDATE OF label, heading, body
    ON legal_unit
    FOR EACH ROW
EXECUTE PROCEDURE tsvector_update_trigger(
        search_vector,
        'vademecum.portuguese_unaccent',
        label,
        heading,
        body
                  );
...
CREATE OR REPLACE FUNCTION jurisprudence_decision_update_search_vector()
    RETURNS trigger
    LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_jurisprudence_decision_search_vector
    BEFORE INSERT OR UPDATE OF
        ...
        holding,
        full_text
    ON jurisprudence_decision
    FOR EACH ROW
EXECUTE PROCEDURE jurisprudence_decision_update_search_vector();
